...
void AMenuSystemCharacter::OnFindSessionsComplete(bool bWasSuccessful)
{
	...
	for (auto Result : SessionSearch->SearchResults)
	{
		...
		if(MatchType == FString("FreeForAll"))
		{
			...
			{
				GEngine->AddOnScreenDebugMessage(-1,15.f,FColor::Cyan,
					FString::Printf(TEXT("Joining Match Tipe: %s"), *MatchType));
			}
	...
	if(bWasSuccessful)
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1,15.f,FColor::Blue, FString(TEXT("Already find a session!")));
		}
	}
	else
	{
		if(GEngine)
		{
			GEngine->AddOnScreenDebugMessage(-1,15.f,FColor::Red, FString(TEXT("Failed to find session!")));
		}
	}	
}
